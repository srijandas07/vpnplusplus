...
class InceptionI3d(nn.Module):
    ...
    def replace_logits(self, num_classes):
        ...
        self.logits = Unit3D(in_channels=384+384+128+128, output_channels=self._num_classes,
                             ...
                             use_bias=True,
                             name='logits')
        #self.fuse_fc = nn.Linear(in_features=384+384+128+128, out_features=4096)
        self.fc = nn.Linear(in_features=7*self._num_classes,
    ...
    def fuse_layer(self):
        self.fuse_fc = nn.Linear(in_features=7168, out_features=4096)
        self.fuse_att_fc = NONLocalBlock3D(in_channels=1024, bn_layer=True)
        self.emb_fc = nn.Linear(in_features=392*72, out_features=392)
    ...
    def forward(self, x):
        ...
        fuse_x = self.dropout(self.avg_pool(x))
        attention_weights, x = self.fuse_att_fc(x)
        ...
        # print(y.shape)
        y = self.fc(y)
        fuse_y = fuse_x.view(fuse_x.size(0), -1)
        fuse_y = self.fuse_fc(fuse_y)
        return self.emb_fc(attention_weights), y, fuse_y

    def extract_features(self, x):
        self.fuse_fc = nn.Linear(
            in_features=384+384+128+128, out_features=4096)
        for end_point in self.VALID_ENDPOINTS:
            if end_point in self.end_points:
                x = self._modules[end_point](x)
        fuse_x = x
        fuse_y = fuse_x.view(fuse_x.size(0), -1)
        fuse_y = self.fuse_fc(fuse_x)
        return fuse_y
